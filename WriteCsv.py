...
def measure_initial_iterations(iterations_to_measure):
    ...
    for _ in range(iterations_to_measure):
        ...
        WaOA.walrus_optimization_algorithm(name_path_input, ng, ps, alpha[0])
        end_time = time.time()
...
headers = ['num_pop','gbest_value','seed']
results = []
...
total_iterations = len(alpha) * len(num_pop) * reps
    ...
    for rep in range(reps):
        for a in alpha:
            for ng, ps in num_pop:
                ...
                gbest_value = WaOA.walrus_optimization_algorithm(name_path_input, ng, ps, a)
                end_time = time.time()  # End timing
                duration = end_time - start_time  # Calculate duration
                current_num_pop = (ng, ps)
                results.append([current_num_pop,gbest_value,my_seed])  # Include duration in results
                pbar.update(1)

# Writing results to a CSV file
with open(f'WaOA_{name_path_input}_{current_date}.csv', 'w', newline='') as file:
